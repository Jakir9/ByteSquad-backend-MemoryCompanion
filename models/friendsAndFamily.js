...
export async function getFriendsAndFamily(userId) {
  const friendsAndFamily = await pool.query(
    "SELECT friendsandfamilyinfo.fnf_id, fnf_name, fnf_relationship, fnf_age, fnf_dob, fnf_photo FROM friendsandfamilyinfo LEFT JOIN friendsandfamily ON friendsandfamilyinfo.fnf_id = friendsandfamily.fnf_id WHERE user_id = 1"
  );
  // console.log(
